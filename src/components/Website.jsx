...
import { getWebsiteProject } from "../features/projects/projectSlice";

const Website = () => {
  const dispatch=useDispatch();
  const { isLoading, isError, message, websiteClone } = useSelector(state => state.project)
  ...
  if (isLoading)
    return (
      <div className=' flex items-center my-10 justify-center z-0'>
        <h1 className="text-xl font-bold text-white">Website Clone Laoding...</h1>
        <div role="status">
  ...
  return (
    <>
      <div id='UI' className='p-10 border-y border-gray-700'>
        <h2 className="font-bold text-5xl text-center my-5  text-white p-4 ">Websites Clones</h2>
        <div className="flex flex-row flex-wrap    ">

          {
            websiteClone.map((data, index) => (
              <div key={index} style={gradientButton} className="max-w-sm mx-auto m-3 rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                ...
                <div className="p-5">
                  <h2 className="text-xl font-semibold text-gray-300 mb-1">{data.title}</h2>
